 ...
 */
function extractNixOSConfigurations(content) {
 ...
 */
function hasPackagesOutput(content) {
    return /packages\s*=/.test(content);
}
/**
 * Check if flake has devShells output
 */
function hasDevShellsOutput(content) {
 ...
 */
export async function parseFlake(flakePath) {
    ...
    try {
        ...
        throw new Error(`Cannot read flake.nix at ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
 ...
 */
export async function getNixOSHosts(flakePath) {
 ...
 */
export async function hasNixOSHost(flakePath, hostName) {
    const hosts = await getNixOSHosts(flakePath);
    return hosts.includes(hostName);
}
//# sourceMappingURL=flake-parser.js.map