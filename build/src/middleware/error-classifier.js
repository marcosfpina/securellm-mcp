...
export class ErrorClassifier {
     ...
     */
    static isRateLimitError(err, message) {
     ...
     */
    static isTransientError(err, message) {
        const httpStatus = this.extractHttpStatus(err);
    ...
    static isPermanentError(err, message) {
        ...
        return (permanentStatuses.includes(httpStatus || 0) ||
            permanentKeywords.some(keyword => message.includes(keyword)));
    }
    /**
     * Check if error is a server error (5xx)
     */
    static isServerError(err, message) {
        const httpStatus = this.extractHttpStatus(err);
     ...
     */
    static extractErrorCode(err) {
        if (typeof err.code === 'string')
            return err.code;
        if (typeof err.errorCode === 'string')
            return err.errorCode;
        if (typeof err.response?.code === 'string')
