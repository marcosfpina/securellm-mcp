...
export class SSHTunnelManager {
    tunnels = new Map();
    connectionManager;
    servers = new Map(); // For SOCKS and remote tunnels
    streams = new Map(); // Track active streams per tunnel
    ...
    async createLocalTunnel(config) {
        ...
        try {
            ...
            const server = net.createServer((socket) => {
                ...
                // Forward through SSH
                conn.client.forwardOut(bindAddress, config.local_port, config.remote_host, config.remote_port, (err, stream) => {
                    if (err) {
    ...
    async createRemoteTunnel(config) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                conn.client.forwardIn(bindAddress, config.remote_port, (err) => {
                    if (err) {
                        reject(err);
                    }
                    else {
            ...
            const tcpConnectionHandler = (info, accept, reject) => {
            ...
            tunnel.tcpConnectionHandler = tcpConnectionHandler;
            return {
                success: true,
                data: {
                    tunnel_id: tunnelId,
                ...
                socket.once('data', (data) => {
                    ...
                    if (data[0] === 5) {
                        ...
                        socket.once('data', (data) => {
                            ...
                            // Forward through SSH
                            conn.client.forwardOut('0.0.0.0', 0, dstAddr, dstPort, (err, stream) => {
                                if (err) {
     ...
     */
    async listTunnels(connection_id) {
    ...
    async closeTunnel(tunnel_id) {
        ...
        if (tunnel.config.type === 'remote') {
            ...
            if (conn && tunnel.tcpConnectionHandler) {
                conn.client.removeListener('tcp connection', tunnel.tcpConnectionHandler);
            }
        }
     ...
     */
    async getTunnelStatus(tunnel_id) {
        const tunnel = this.tunnels.get(tunnel_id);
        if (!tunnel) {
            throw new Error(`Tunnel not found: ${tunnel_id}`);
        }
     ...
     */
    async reconnectTunnel(tunnelId) {
        const tunnel = this.tunnels.get(tunnelId);
        if (!tunnel) {
        ...
        try {
            ...
            if (tunnel.config.type === 'local') {
                result = await this.createLocalTunnel(tunnel.config);
            }
            else if (tunnel.config.type === 'remote') {
                result = await this.createRemoteTunnel(tunnel.config);
            }
            else {
                result = await this.createDynamicTunnel(tunnel.config);
            }
            if (result.success) {
