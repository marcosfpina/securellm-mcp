# Infrastructure Tools Type Specification

## 📋 Overview

This document provides comprehensive TypeScript type definitions for all infrastructure management tools in the SecureLLM Bridge MCP server. These types ensure type safety, enable IntelliSense support, and enforce consistent interfaces across all infrastructure operations.

---

## 🏗️ Core Infrastructure Types
...
export type NASOperation =
  ...
  | { type: 'get_quota'; volume: string; type: QuotaType; target: string }
...
export type ClusterOperation =
  ...
  | { type: 'scale_workload'; cluster_type: ClusterType; name: string; namespace?: string; replicas: number }
...
export function requiresSudo(operation: any): boolean {
  // Check if operation requires sudo privileges
  return operation.sudo_required === true;
}
```
...
```typescript
export function isNFSMountOperation(op: NFSOperation): op is { type: 'mount'; config: NFSMountConfig } {
  return op.type === 'mount';
}

export function isNFSExportOperation(op: NFSOperation): op is { type: 'add_export'; config: NFSExportConfig } {
...
```typescript
export function isNASVolumeOperation(op: NASOperation): op is { type: 'create_volume'; config: NASVolumeConfig } {
  return op.type === 'create_volume';
}

export function isNASSnapshotOperation(op: NASOperation): op is { type: 'create_snapshot'; config: NASSnapshotConfig } {
...
All types defined in this specification should be exported from the main infrastructure types file:

```typescript
// src/types/infrastructure.ts
export * from './infrastructure/execution';
export * from './infrastructure/nfs';
export * from './infrastructure/nas';
export * from './infrastructure/cluster';
...
**Status**: Type Specification Complete  
**Total Types**: 150+ type definitions, interfaces, and discriminated unions